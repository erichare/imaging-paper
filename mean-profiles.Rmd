...
br115 <- "~/CSAFE/Bullets/Hamby252_3DX3P1of2/Br1 Bullet 2-1.x3p"
...
mean_profile_500 <- gg115 %>% filter(ccf > .975, between(x,50,500)) %>% group_by(y) %>%
  summarize(
    resid = mean(resid)
  )

mean_profile_300 <- gg115 %>% filter(ccf > .975, between(x,50,300)) %>% group_by(y) %>%
  summarize(
    resid = mean(resid)
  )
```
...
```{r, echo=FALSE}
qplot(y, resid, data=mean_profile_all, geom="line") + theme_bw() +
  geom_line(aes(y, resid), data=mean_profile_300, colour="green") +
  geom_line(aes(y, resid), data=mean_profile_500, colour="blue") +
  geom_line(aes(y, resid), data=gg115 %>% filter(x==100), colour="red") + ylim(c(-5,5))
