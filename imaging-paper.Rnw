...
\hh{In this instance, the radius is estimated as $\hat{r} = \Sexpr{pars$radius}\mu m = \Sexpr{pars$radius/1000}mm$, which is consistent with a 9 mm bullet fired in a Ruger P-85. }
...
qplot(y, resid, data=subset(db2, x == 100), #colour=factor(x), 
  ...
  ggtitle("Land 5 of Bullet 1 in black")
@
\caption{Circle fit for each cross section of bullet 2, with land 5 of bullet 1 overlaid.\label{fig:circlefits}}%
\end{figure}

\subsection{Loess Fit}

To improve upon the previous algorithm, the first step must be to eliminate the grooves from the images under the assumption that they do not contain relevant information for determining a match. Fortunately, the location and appearance of the grooves in these 2d plots is quite consistent. By noting that height values beginning at $y = 0$ reach a maximum around the peak of the groove, we can then follow the descent of the height values to valley of the groove. This marks the point at which we should trim the image. We repeat a similar procedure for the right-hand groove by starting at the maximum y value and working backwards. The procedure can be described as follows:

\begin{enumerate}
    \item Fix an $x$ coordinate value (Figure~\ref{fig:loess_step1}).
    \item For each $y$ value, compute a rolling average with a particular \textbf{smoothing factor} to smooth out any deviations occurring near the minima. (Figure~\ref{fig:loess_step2}).
    \item For each smoothed $y$ value, compute another rolling average using the same smoothing factor above. (Figure~\ref{fig:loess_step3}).
    \item Determine the peak of the left groove by finding the first doubled-smoothed value $y_i$ such that $y_i > y_{i - 1}$ and $y_i > y_{i + 1}$. Call this peak $p_{left}$. (Figure~\ref{fig:loess_step47}).
    \item Determine the valley of the left groove by finding the first doubled-smoothed $y_j > p_{left}$ such that $y_j < y_{j - 1}$ and $y_j < y_{j + 1}$. Call this valley $v_{left}$.
    ...
    \item Remove all $y_i < v_{left}$ and $y_i > v_{right}$. (Figure \ref{fig:loess_step47}).
\end{enumerate}

\begin{figure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step1}Step 1 the Loess fit algorithm. For a fixed crosscut (x held constant at 243.75) height values for land 5 of bullet 1 are plotted across different values of y.}
...
%\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step3}Steps 2 \& 3 of the Loess fit algorithm. The  height values are smoothed twice with a smoothing factor of 35.}
<<loess_step3, echo=FALSE, fig.width=10, fig.height=3.5, out.width='\\linewidth', warning=FALSE,  message=FALSE>>=
xloc=250
p3 <- ggplot() +
  annotate("rect",xmin=xloc-17*1.5625, xmax=xloc+18*1.5625, ymin=45, ymax=180, fill=alpha("orange", alpha=0.5)) + 
  geom_point(aes(x=(1:length(smoothed_truefalse))*1.5625, y=smoothed_truefalse)) + theme_bw() +
    geom_vline(xintercept=xloc, colour="grey50") + xlab("y") + ylab("")
...
\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step47}Steps 4 - 7 of the Loess fitting algorithm. After smoothing the $y$ values left and right minima are detected (marked by vertical lines, red indicating peaks and blue indicating valleys). Values outside these boundaries are removed (shown in grey)}
...
@
\end{subfigure}
\caption{Overview of all seven steps of the smoothing algorithm to identify and remove grooves from the bullet images.}
\end{figure}

The ``smoothing factor" described in the algorithm represents the window size to use for a rolling average. Higher values of the smoothing factor therefore lead to more smoothed results. In the groove detection portion of this algorithm, we wish to remove most of the deviation but still maintain enough signal in the groove to detect it. Empirically, a window of 35 observations for the smoothing factor seems to work quite well.
%currently a window of 57.8375$\mu m$ is used, corresponding to a window of 17 values to the left and the right of an observation
It is important to note that the smoothing pass is done twice. That is, the smoothed data is once again smoothed by computing a new rolling average with the same smoothing factor. This bears some similarities to the ideas of John Tukey in his book Exploratory Data Analysis, where he describes a smoothing process called ``twicing" in which a second pass is made on the residuals computed from the first pass \cite{tukey:1977}. \hh{Did Tukey say, what the goal of the twicing was? - also, do you have a page number for the twicing?}



Next, we fit a Loess regression to the data. Figure \ref{fig:loess_fit} provides a look at the Loess fit, in blue, overlaid against the processed image of land 5 of bullet 1. The fit seems to do a reasonable job of capturing the structure of the image. Figure \ref{fig:loess_resid} shows the residuals from this fit. It is clear that there is a rather significant amount of autocorrelation, which make sense given the presence of striations and markings. Certainly, tuning the parameters for this regression, such as the Loess span, could prove to do an even better job at making the markings more clear. \hh{XXX did you follow up on that? What is the effect of using a different span?}

\begin{figure}[hbtp]
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_fit} Loess fit for land 5 of bullet 1.}
...
\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_resid} Residuals of Loess fit for land 5 of bullet 1.}
<<loess_resid, echo=FALSE, fig.width=10, fig.height=3.5, out.width='\\linewidth', warning=FALSE>>=
my.loess$resid + ylab("Residual of Loess Fit")
@
\end{subfigure}
\caption{Fit and residuals of a loess fit to land 5 of bullet 1 (Barrel 1).}
\end{figure}

We can reproduce a similar figure to the one from the circular fits by overlaying the residuals from each loess fit for bullet 2 onto an image of the loess residuals from land 5 of bullet 1. This is shown in Figure~\ref{fig:loessfits}. 
...
qplot(y, resid, data=melt.df, #colour=factor(x), 
  ...
  facet_wrap(~part, ncol=2) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_line(data = actual.df, aes(y, resid), colour="black", size=.75, alpha=0.5) + ggtitle("Land 5 of Bullet 1 in black")
@    
\caption{\label{fig:loessfits} Residuals of a loess fit to bullets 1 and 2 from barrel 1. Land 5 of bullet 1 is shown in black. }
\end{figure}

\section{Application}
We have implemented a web interface to automate many of the steps outlined in the previous section. The web interface, developed using Shiny \cite{shiny}, allows a user to upload two x3p files representing scans of a bullet between two grooves. A 3D viewer, using Plotly \cite{plotly}, allows the user to rotate and zoom to examine surface features of the images. A screenshot of the application is shown in Figure~\ref{fig:app}. The application is available at the url \url{http://erichare.shinyapps.io/x3prproto}.

\begin{figure*}
\includegraphics[width=\textwidth]{images/app.png}
\caption{\label{fig:app}{Screenshot of the web interface for analyzing bullet images. \hh{The shot shows two lands from the Hamby study. Although the lands are a match, this is visually very hard to confirm.}}}
\end{figure*}
...
\caption{\label{fig:manualmatch}\hh{Manually adjusted side-by-side comparison of land 5 of bullet 1 and land 1 of bullet 2.}}
\end{figure}
\begin{figure}
\includegraphics[width=\columnwidth]{images/matchup-rgl.png}
\caption{\label{fig:manualmatch-rgl}\hh{Different view of the manually adjusted side-by-side comparison of land 5 of bullet 1 and land 1 of bullet 2.}}
\end{figure}
...
qplot(data=lofX, y, resid, colour=bullet, size=I(0.75), geom="line", group=bullet) +
  ...
  theme_bw() + ylab("Residuals of Loess Fit") + 
  theme(legend.position="none")
@
    }
\end{subfigure}
...
<<smoothcut, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=4, out.width='\\textwidth', warning=FALSE>>=
  ...
  p1 <- ggplot() + theme_bw() +
    geom_rect(aes(xmin=xmin, xmax=xmax, fill=factor(type)), ymin=-6, ymax=6, data=peaks1$lines,  alpha=0.2) +
    geom_vline(aes(xintercept=extrema, colour=factor(type)), data= peaks1$lines) + scale_colour_brewer(palette="Set2") + 
    scale_fill_brewer(palette="Set2") +
    theme(legend.position="none") + 
    geom_line(aes(x=y, y=l30, group=bullet), data=subset(lofX, bullet==b12[1])) +
  ylab(expression(paste("Crosscut values (in ",mu,"m)", sep=""))) +
    ggtitle(b12[1])+ xlim(c(0, 2000)) 

  p2 <- ggplot() + theme_bw() +
    geom_rect(aes(xmin=xmin, xmax=xmax, fill=factor(type)), ymin=-6, ymax=6, data=peaks2$lines,  alpha=0.2) +
    geom_vline(aes(xintercept=extrema, colour=factor(type)), data= peaks2$lines) + scale_colour_brewer(palette="Set2") + 
    scale_fill_brewer(palette="Set2") +
    theme(legend.position="none") + 
    geom_line(aes(x=y, y=l30, group=bullet), data=subset(lofX, bullet==b12[2])) +
  ylab(expression(paste("Crosscut values (in ",mu,"m)", sep=""))) +
    ggtitle(b12[2]) + xlim(c(0, 2000))
...
ggplot() + 
  ...
  geom_line(data=lofX, aes(x=y, y=l30, group=bullet, colour=bullet)) +
  scale_colour_brewer("", palette="Set1") +
...
Figure~\ref{fig:match} gives an overview of the rest of the automated matching routine: after smoothing the scan with a loess of a very small span, shown in (a), we define a striation mark as a groove or a peak if \hh{XXX }
...
The crosscut at which a comparison is done matters -- as crosscuts are further apart, more pronounced differences between the crosscuts show up. For bullet land 1-5 from barrel 1 crosscuts at intervals of 50$\mu m$ between 150$\mu m$  and 450$\mu m$  are compared to a crosscut at 100$\mu m$. The initially almost perfect match (consolidated by a CMS of fourteen) between the  crosscuts deteriorates with increasing distance to non-matches at a crosscut of $x = 300\mu m$ or higher. Only if the crosscut is within 150$\mu m$ do we get a good visual match. Given that alignment in the microscope is not perfect, we should take this into account in the automatic matching routine by evaluating matches at several cross sections. 
\begin{figure}
...
\hh{The automated matching routine is then as follows:
Identify a stable region for each bullet and pick the lowest crosscut level in this region (assumption here: lower crosscut levels show better expression of individual marks XXX need to still check)

All of the other steps are done on pairs of bullets:
\begin{enumerate}
\item Smooth the two crosscuts by a loess with a very small span. 
\item Use cross-correlation to find the best alignment along the crosscuts: Shift one of the crosscuts by the lag indicated by the cross-correlation function.
\item Using a rolling average, identify peaks and grooves for each of the crosscuts. We define a region around the extrema on each side as one third of the distance to the next extrema. 
%Based on a starting threshold level of $\theta$, identify areas of the crosscuts whose absolute value are above the thresholds. These areas are regions of potential matches between the crosscuts. If all of the values of the crosscuts stay on the same side, the area is called a matching line. If only one crosscut exceeds the threshold, or if the two crosscuts move into opposite directions over the threshold, the area is a non-match.
\item Match grooves and peaks between crosscuts. \hh{XXX  needs some more details.} 
...
grid.arrange(list_of_matches[[1]], list_of_matches[[2]], list_of_matches[[3]], 
             ...
             ncol = 1)
@
\caption{\label{fig:barrel2}All lands from the first bullet shot from barrel 2 are compared to the first land of the second bullet shot from barrel 2 (in blue). The largest number of CMS is eleven, observed with land six of the second bullet. }
\end{figure}
...
\hh{We can use the Hamby study to run all unkowns against the known bullets. When things go perfectly, they look like the results in Figure~\ref{fig:hamby-perfect}.}
...
\end{figure}
\hh{XXX a lot more about the Hamby study. Fifteen unknown bullets (i.e. 90 lands) compared to two times ten known bullets (i.e. 120 lands) makes $90 \times 120 = 10,800$ land to land comparisons, out of which 180 are actual matches. It is not reasonable to assume that we can match all lands, but we can try to maximize the number of matches. }
Figure~\ref{fig:cms} shows the strong connection between the maximal number of consecutive striae and matches in the Hamby study. All 23 pairs of lands with at least twelve CMS in common are matches. For smaller number of CMS, the percentage of false positives quickly increases. Taking cross-correlation into account (see Figure~\ref{fig:ccfs-match}) increases the number of matches. A cross-correlation of above 0.75 in the Hamby study is indicative of a match. There are 67 land-land comparisons with a cross-correlation of above 0.75.   
\begin{figure}
\begin{subfigure}[t]{\linewidth}
\caption{Distribution of maximal CMS.}
<<cms-bars, echo=FALSE, fig.width=7, fig.height=3.5, out.width='\\linewidth'>>=
bstats <- read.csv("data-35/bullet-stats.csv")
ggplot(data=bstats) + 
...
\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{All land-to-land comparisons a maximum number of CMS of twelve or more are matches.}
...
ggplot(data=bstats) + 
  ...
  theme(legend.position="none") +
    xlab("maximum CMS")
@
\caption{If the distance between the crosscuts is small, a smaller number of CMS might already be an indication of a match.}
\end{figure}

\begin{figure}
<<ccfs-match, echo=FALSE, fig.width=8, fig.height=4, out.width='\\linewidth'>>=
ggplot(data=bstats) + theme_bw() + 
  geom_jitter(aes(x=factor(cms), y=ccf, colour=match), alpha=.5) + facet_wrap(~match, labeller="label_both") + xlab("maximum CMS") + ylab("Cross-correlation") +
  theme(legend.position="none")
@
\caption{\label{fig:ccfs-match}If the cross-correlations between the crosscuts is high, a smaller number of CMS might already be an indication of a match.}
\end{figure}



\hh{XXX another idea for the rest of the paper: use some datamining methods to match the Hamby bullets }
Using recursive partitioning, we find a decision tree \cite{breiman:1984, rpart, rpart.plot} predicting matches based on features derived from the image files. The resulting tree is shown in Figure~\ref{fig:tree}. A total of 105 lands is being matched correctly. Interestingly, the number of consecutive matching striae is not featuring as an important variable in this evaluation. Instead, cross-correlation (ccf) between the crosscuts is very predominant. Aside from cross correlation, the number of matches (num.matches) and the cumulative height of the joint peaks (sumpeaks) between crosscuts is involved in the decision. Within matches and non-matches, the values of CMS and cross-correlation seem to be almost independent (see Figure~\ref{fig:ccfs-match}), but both are highly predictive in distinguishing matches from non-matches. Out of the two, cross-correlation has higher  predictive power. This  does not  contradict earlier findings emphasizing the value of CMS on visual assessments of bullet matches: in those papers, assessments were based on purely visual inspection of either actual bullets or 2d microscopic images of bullets.
...
prp(rp1, extra = 101,  box.col=c("grey90", "orange")[as.numeric(per > 0.5)+1])
...
<<pred:table, echo=FALSE, results='asis'>>=
library(xtable)
xtable(xtabs(~I(pred>0.75)+match, data=CCFs))
@
...
@
\caption{\label{fig:treeresult}Overall predictions for each bullet based on the decision tree. For each bullet at least one land is correctly matched. Unknown bullet B is the hardest for the algorithm to match.}
\end{figure}
...
%' qplot(1:nrow(rtrees$err.rate), OOB, geom="line", data=data.frame(rtrees$err.rate)) + theme_bw() + ylim(c(0,NA))
%' CCFs$forest <- predict(rtrees, type="prob")[,2]
%' importance(rtrees)
%' qplot(pred, forest, geom="jitter", data=CCFs, colour=match)
%' @
%' 
...
\item The matching algorithm is sensitive to the parameter choices made. The distance between crosscuts (currently 25$\mu m$) to evaluate stability, as well as the cross-correlation factor (currently 0.9) we set as a minimum are affecting the final outcome. Another parameter is the amount of smoothing in identifying peaks and grooves (currently a window of 35 observations is used). While we tried to lay out in the paper the impact each of the parameter choices has on the matching, we are still far from an optimized scenario. 

\item involve multiple crosscuts for each bullet in a match?
\item assessing different crosscuts at the same bullet allows us to assess a land's quality in terms of how stable it is -- generally, break-off patterns are less stable. 
