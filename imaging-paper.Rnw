...
<<smoothcut, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=4, out.width='\\textwidth', warning=FALSE>>=
  ...
  p1 <- peaks1$plot + xlab("y") + ylab("smoothed crosscut") + ggtitle(b12[1])
  p2 <- peaks2$plot + xlab("y") + ylab("smoothed crosscut") + ggtitle(b12[2])
...
ggplot() + 
  geom_rect(aes(xmin = xmin, xmax = xmax, fill=factor(type)), ymin = -6, ymax=6,  data = lines, alpha=0.2) +
  theme(legend.position="bottom") +
  ...
  theme(legend.position="none") +
  ylab(expression("Crosscut values in",mu,m)) +
  xlab("y")
...
reslist <- lapply(crosscuts[-1], function(cc) {
  ...
  list(maxCMS = maxCMS, ccf = bAlign$ccf, threshold=threshold, lines=lines, bullets=lofX)
})
...
reslist <- lapply(1:length(crosscuts[-1]), function(i) {
  ...
  lines <- striation_identify(lofX, threshold = threshold)
  maxCMS <- maxCMS(lines$match==TRUE)
  list(maxCMS = maxCMS, threshold=threshold, ccf = bAlign$ccf, lines=lines, bullets=lofX)
})
