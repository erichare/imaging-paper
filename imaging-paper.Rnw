...
Using recursive partitioning, we find a decision tree \cite{breiman:1984, rpart, rpart.plot} predicting matches based on features derived from the image files. The resulting tree is shown in Figure~\ref{fig:tree}. A total of 93 lands is being matched correctly. Interestingly, the number of consecutive matching striae is not featured in this evaluation. Instead, cross-correlation (ccf) between the crosscuts is featured heavily. Aside from cross correlation, the number of matches between crosscuts is involved in the decision. Within matches and non-matches, the values of CMS and cross-correlation seem to be almost independent (see Figure~\ref{fig:ccfs-match}), but both are highly predictive in distinguishing matches from non-matches. Out of the two, cross-correlation is the variable with more  predictive power. This  does not  contradict earlier findings emphasizing the value of CMS on visual assessments of bullet matches: in those papers, assessments were based on purely visual inspection of either actual bullets or 2d microscopic images of bullets.
...
prp(rp1, extra = 101, fallen.leaves=TRUE, box.col=c("grey90", "orange")[as.numeric(per > 0.5)+1])
...
\paragraph{Acknowledgment}: thanks to the men and women behind R\cite{R}, and the authors of the packages  knitr \cite{knitr} and ggplot2 \cite{ggplot2}.
\section{Conclusion}
