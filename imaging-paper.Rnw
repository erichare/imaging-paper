...
\begin{figure}
<<tree:forest, echo=FALSE, fig.width=5, fig.height=6, out.width='\\linewidth'>>=
qplot(pred, forest, geom="point", data=CCFs, colour=match) + theme_bw() +
  theme(legend.position="bottom")
@
\caption{\label{fig:tree-forest}Scatterplot of prediction results from the tree (horizontal) and the forest (vertical). Each point corresponds to a land-to-land comparison. Matches are indicated in blue.}
\end{figure}
