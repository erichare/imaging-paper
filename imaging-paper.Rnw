...
The crosscut at which a comparison is done matters -- as crosscuts are further apart, more pronounced differences between the crosscuts show up. For bullet land 1-5 from barrel 1 crosscuts at intervals of 50$\mu m$ between 150$\mu m$  and 450$\mu m$  are compared to a crosscut at 100$\mu m$. The initially almost perfect match (consolidated by a CMS of nineteen) between the  crosscuts deteriorates with increasing distance to a very questionable match at $x = 350\mu m$ and non-matches further apart. Only if the crosscut is within 200$\mu m$ do we get a good visual match. Given that alignment in the microscope is not perfect, we should take this into account in the automatic matching routine by evaluating matches at several cross sections. \hh{XXX We're not doing that yet, but it might help - particular for bullets that have larger break-offs at the bottom.}
\begin{figure}
...
\hh{We can use the Hamby study to run all unkowns against the known bullets. When things go perfectly, they look like in Figure~\ref{fig:hamby-perfect}.}
