...
Another benefit is that we can not only apply a single decision tree, but several hundred of them to combine in a random forest \cite{breiman:2001, randomForest}. This allows us to quantify XXXX
...
imp$sdNonMatch <- sprintf("(%6.2f)", imp$sdNonMatch)

names(imp)[-(1:2)] <- c("Match", "(sd)", "Non-Match", "(sd)")
print(xtable(imp, align="clrrlrl"), include.rownames=FALSE, floating = FALSE)
@
