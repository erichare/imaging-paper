...
\journal{Journal of Forensic Science International}
...
\tnotetext[mytitlenote]{Resources for this manuscript, including R code to reproduce any of the models, tables and figures, can be found at  \url{https://github.com/erichare/imaging-paper}.}

%% Group authors per affiliation:
\author{Eric Hare, Heike Hofmann, Alicia Carriquiry}
\address{Department of Statistics and Statistical Laboratory, Iowa State University}
...
\ead{erichare@iastate.edu}

% \address[mymainaddress]{Department of Statistics and Statistical Laboratory, Iowa State University}
% \address[mysecondaryaddress]{secondary address -- CS?}
...
\begin{keyword}
\hh{XXX not the right keywords yet} 3d Topological Surface Measurement, Data visualization, Machine Learning, Feature importance.
\end{keyword}
...
% \item \hh{XXX Alternative approaches: IBIS database and the NRC report\citep{nap:2008} against it\footnote{Fox report on the NRC report \url{http://goo.gl/XurSVl}.
...
<<residual2, dependson='fixedX', echo=FALSE, warning=FALSE, fig.height=3, out.width='.7\\linewidth'>>=
...
<<residual, dependson='fixedX', echo=FALSE, warning=FALSE, fig.height=3, out.width='.7\\linewidth'>>=
...
<<loess_step1, echo=FALSE, fig.width=10, fig.height=3.5, out.width='.7\\linewidth', warning=FALSE,  message=FALSE>>=
...
<<loess_step3, echo=FALSE, fig.width=10, fig.height=3.5, out.width='.7\\linewidth', warning=FALSE,  message=FALSE>>=
...
<<loess_step47, echo=FALSE, fig.width=10, fig.height=3.5, out.width='.7\\linewidth', warning=FALSE,  message=FALSE>>=
...
<<loess_fit, echo=FALSE, fig.width=10, fig.height=3.5, out.width='.7\\linewidth', warning=FALSE,  message=FALSE>>=
...
<<loess_resid, echo=FALSE, fig.width=10, fig.height=3.5, out.width='.7\\linewidth', warning=FALSE>>=
...
<<crosscuts, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3, out.width='.7\\textwidth'>>=
...
<<crosscutsX, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3, out.width='.7\\textwidth'>>=
...
\begin{figure}[hbtp]
<<ccf, echo=FALSE, fig.width = 7, fig.height = 3, out.width = '.7\\linewidth'>>=
lag <- ccf$lag[which.max(ccf$acf)]
...
\begin{figure}[hbtp]
<<crosscuts-vary, echo=FALSE, fig.width = 7, fig.height = 14, out.width = '.7\\linewidth', cache=TRUE, warning = FALSE>>=
paths <- file.path(datadir, dir(datadir))
...
\begin{figure}[hbtp]
<<crosscuts-vary-b31, echo=FALSE, fig.width = 7, fig.height = 10, out.width = '.7\\linewidth', cache=TRUE, warning = FALSE>>=
paths <- file.path(datadir, dir(datadir))
...
<<smooth, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3.5, out.width='.7\\textwidth', warning=FALSE>>=
...
<<smoothcut, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3.5, out.width='.7\\textwidth', warning=FALSE>>=
#images <- file.path(datadir, dir(datadir))
  lof <- processBullets(paths = images[c(5,7)], x = 100)
...
<<smoothmatch, echo=FALSE, dependson='twolands100match', fig.width=7, fig.height=2.75, out.width='.7\\textwidth', warning=FALSE>>=
peaks1$lines$bullet <- b12[1]
peaks2$lines$bullet <- b12[2]
...
\begin{figure}[hbtp]
<<smoothfac1, echo=FALSE, fig.width=7, fig.height=5.2, out.width='.7\\linewidth', warning=FALSE>>=
br111.groove <- get_grooves(br111_bullet)
...
\begin{figure}[hbtp]
<<barrel2, echo=FALSE, fig.width = 7, fig.height=12, out.width='.7\\linewidth', warning=FALSE>>=
...
<<cms, echo=FALSE, fig.width=7, fig.height=3.5, out.width='.7\\textwidth'>>=
...
<<top, echo=FALSE, fig.width=7, fig.height=3.25, out.width='.7\\textwidth', warning = FALSE>>=
...
<<top2, echo=FALSE, fig.width=7, fig.height=3.25, out.width='.7\\textwidth', warning = FALSE>>=
...
<<cms-bars, echo=FALSE, fig.width=7, fig.height=3.5, out.width='.7\\linewidth'>>=
...
<<cms-spines, echo=FALSE,  fig.width=7, fig.height=3.5, out.width='.7\\linewidth'>>=
ggplot(data=subset(bstats, !flagged)) + 
...
<<cms-densities, echo=FALSE,  fig.width=7, fig.height=3.5, out.width='.7\\linewidth'>>=
x <- seq(0,29, by=0.03)
...
\begin{figure}[hbtp]
<<strange-res, echo=FALSE, fig.width=7, fig.height=3.25, out.width='.7\\linewidth', warning=FALSE>>=
...
<<distr.dist, echo=FALSE, fig.width=8, fig.height=4, out.width='.7\\linewidth'>>=
ggplot(data=bstats) + 
...
<<crosscut-densities, echo=FALSE,  fig.width=7, fig.height=3.5, out.width='.7\\linewidth'>>=
ggplot(data=bstats) + 
...
<<ccfs-match, echo=FALSE, fig.width=8, fig.height=4, out.width='.7\\linewidth'>>=
...
\begin{subfigure}[t]{\linewidth}
\caption{Densities of distances between cross sections for Known Matches (KM) and Known Non Matches (KNM).}
<<ccf-densities, echo=FALSE,  fig.width=7, fig.height=3.5, out.width='.7\\linewidth'>>=
ggplot(data=bstats) + 
...
<<ccf-ratio, echo=FALSE,  fig.width=7, fig.height=3.5, out.width='.7\\linewidth'>>=
x <- seq(0,1, by=0.001)
library(sm)
...
\begin{figure}[hbtp]
<<tree, echo=FALSE, fig.width=7, fig.height=4, out.width='.7\\linewidth'>>=
library(rpart)
library(rpart.plot)
library(RColorBrewer)
...
rp1 <- rpart(match~., bstats[-excludesObs,includesVar])  # doesn't include cms at all !!!!
...
library(reshape2)
set.seed(20151215)
rtrees <- randomForest(factor(match)~., data=bstats[-excludesObs,includesVar], ntree=300)
...
%
\begin{figure}[hbtp]
<<oob, echo=FALSE, fig.width=7, fig.height=3, out.width='.7\\linewidth'>>=
...
\begin{figure}[hbtp]
<<overall-tree, echo=FALSE, fig.width=7, fig.height=4, out.width='.7\\linewidth'>>=
...
\begin{figure}[hbtp]
<<tree:forest, echo=FALSE, fig.width=6, fig.height=6.25, out.width='.7\\linewidth', warning = FALSE>>=
