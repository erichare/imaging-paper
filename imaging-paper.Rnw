...
\begin{enumerate}
    ...
    \item For each $y$ value, smooth out any deviations occurring near the minima by twice applying a rolling average with a pre-set \textbf{smoothing factor}. (Figure~\ref{fig:loess_step3}).
...
%\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step3}Step 2 of the Loess fit algorithm. The  height values are smoothed twice with a smoothing factor of 35.}
...
It is important to note that the smoothing pass is done twice. That is, the smoothed data is once again smoothed by computing a new rolling average with the same smoothing factor. This bears some similarities to the ideas of John Tukey in his book Exploratory Data Analysis, where he describes a smoothing process called ``twicing" in which a second pass is made on the residuals computed from the first pass \cite{tukey:1977}. \hh{XXX Did Tukey say, what the goal of the twicing was? - also, do you have a page number for the twicing?}

Next, we fit a Loess regression to the data. Figure \ref{fig:loess_fit} provides a look at the Loess fit in blue, overlaid against the processed image of land 5 of bullet 1. The fit seems to do a reasonable job of capturing the structure. Figure \ref{fig:loess_resid} shows the residuals from this fit. \hh{XXX what purpose does the next paragraph have? I think you just want end this section, but it brings up two new ideas: autocorrelation and dependency on the span. The span obviously doesn't get any resolution ... so it's just muddying the waters at the moment ... XXX}
It is clear that there is a rather significant amount of autocorrelation, which makes sense given the presence of striations and markings. Certainly, tuning the parameters for this regression, such as the Loess span, could prove to do an even better job at making the markings more clear. \hh{XXX did you follow up on that? What is the effect of using a different span?}
