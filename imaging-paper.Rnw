...
Firearm examination is a forensic tool used to determine whether two bullets were fired from the same gun barrel. This process has broad applicability in terms of the conviction  in the United States criminal justice system. Ballistics identification has long been considered an accepted and reliable procedure, but in the past ten years has undergone more significant scrutiny. In 2005, in \emph{United States vs. Green}, the court ruled that the forensic expert could not confirm that the bullet casings came from a specific weapon with certainty, but could merely ``describe" other casings which are similar. Further court cases in the late 2000s expressed caution about the use of ballistics identification evidence~\citep{giannelli:2011}.
...
\hh{XXX Alternative approach by \citet{roberge:2006}: using correlation to identify matches between pairs of bullets using a correlation measure, that `could be automated'. No algorithm implemented/available, no error rates of automated matching are given XXX}
\hh{XXX Interesting paper by \citet{ma:2004} with  emphasis on ccf and acf. They are using a stylus instrument for measureing the big curvature XXX -- the advantage of our approach is that nobody needs to look at anything until after the matching is done.  }
\hh{XXX Hamby study: 507 participants returned results. If a match was reported, these matches were correct. But eight (out of $15 \times 507 = 7,605$) bullets were not matched conclusively, resulting in a false negative rate of 0.0011.}
\hh{XXX for Hamby study: automated systems were successfully used for matching:
...
We then proceed to describing an automatic matching routine which we evaluate on a small database of reference bullets.
...
\caption{\label{fig:loess_step1}Step 1 of the loess fit algorithm. For a fixed cross section ($x$ held constant at 243.75~$\mu m$)  surface measurements for bullet~1-5 are plotted across the range of $y$.}
...
\end{subfigure}
\begin{subfigure}[t]{\textwidth}\centering
\caption{\label{fig:loess_step3}Step 2 of the loess fit algorithm. The  surface measurements are smoothed twice with a smoothing factor of $s = 35$. The orange rectangle shows an example of the smoothing window. Valleys and peaks are detected, if they are not within the same window.}
...
\end{subfigure}
\begin{subfigure}[t]{\textwidth}\centering
\caption{\label{fig:loess_step47}Steps 3 -- 6 of the loess fitting algorithm. After smoothing the surface measurements extrema on the left and right are detected (marked by vertical lines, red indicating peaks and blue indicating valleys). Values outside the blue boundaries are removed (shown in grey)}
...
\hh{XXX Introduce Loess formula with all of its details in here. we are using the default of {\tt loess}, which has a span of 0.75 and uses a polynomial of degree 2. XXX}
...
qplot(y, resid, group=bullet, colour=bullet, data=lof, 
      geom="line") + 
  scale_colour_manual("Matches", values=cols) + theme_bw() +
...
\paragraph{Acknowledgment} Thanks to the men and women behind the software R~\citep{R}, and the authors of the R packages  knitr~\citep{knitr} and ggplot2~\citep{ggplot2}.
