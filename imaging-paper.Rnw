...
ggplot(data=bstats) + theme_bw() + 
  ...
  theme(legend.position="none")
@
\caption{If the cross-correlations between the crosscuts is high, a smaller number of CMS might already be an indication of a match.}
\end{figure}
