...
\item Based on a starting threshold level of $\theta$, identify the maximal number of CMS (consecutive matching striae) between the two crosscuts. Vary $\theta$, such that the maximum number of CMS is reached. 
\end{enumerate}
}
