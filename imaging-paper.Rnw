...
@
\caption{Circle fit for each cross section of bullet 2, with land 5 of bullet 1 overlaid.\label{fig:circlefits} \hh{Land 5 of bullet 1 matches best with the first land of the second bullet.}}%
\end{figure}
...
\begin{figure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step1}Step 1 the Loess fit algorithm. For a fixed crosscut ($x$ held constant at 243.75 $\mu m$) of surface measurements for land 5 of bullet 1 are plotted across different values of $y$.}
...
We can reproduce a similar figure to the one from the circular fits by overlaying the residuals from each loess fit for bullet 2 onto an image of the loess residuals from land 5 of bullet 1. This is shown in Figure~\ref{fig:loessfits}. \hh{The first land of bullet 2 is still the best  match for land 5, but now the details of the match are more pronounced.}
...
qplot(y, resid, data=melt.df, #colour=factor(x), 
  ...
  facet_wrap(~part, ncol=2) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_line(data = actual.df, aes(y, resid), colour="black", size=.75, alpha=0.5) + ggtitle("Land 5 of Bullet 1 in black") + ylab("Residuals of Loess Fit")
@    
...
includes <- setdiff(names(CCFs), c("b1", "b2", "data", "resID", "id.x", "id.y", "pred", "forest"))
...
library(xtable)
xtable(xtabs(~I(pred>0.5)+match, data=CCFs))
@
