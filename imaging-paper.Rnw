...
\begin{enumerate}
    \item Fix an x coordinate value. (Top panel of Figure \ref{fig:loess_step12})
    \item Remove values from $y \in \{0, 100\}$ and $y \in \{900, 1000\}$. (Bottom panel of Figure \ref{fig:loess_step12})
    \item For each y value from 100 to 900, compute a rolling average to smooth out any deviations occurring near the minimums. (Top panel of Figure \ref{fig:loess_step34})
    \item Determine the minimum in the first half (corresponding to the left groove) and the minimum in the right half (corresponding to the right groove) (Bottom panel of Figure \ref{fig:loess_step34})
    \item Remove all y values below the minimum and above the maximum (Figure \ref{fig:loess_step5})
\end{enumerate}

Next, we fit a Loess regression to the data. Figure \ref{fig:loess_fit} provides a look at the Loess fit, in blue, overlaid against the processed image of part 1 of bullet 1. The fit seems to do a reaosnable job of capturing the structure of the image. Figure \ref{fig:loess_resid} shows the residuals from this fit. It is clear that there is a rather significant amount of autocorrelation, which make sense given the presence of striations and markings. Certainly, tuning the parameters for this regression, such as the Loess span, could prove to do an even better job at making the markings more clear.

<<loess_step12, echo=FALSE, fig.width=10, fig.height=7, out.width='\\linewidth', warning=FALSE, fig.cap='Steps 1 and 2 of the Loess fit algorithm. The top panel is a plot of height values for part 1 of bullet 1 across different values of y, with x held constant at 243.75. The bottom panel is the same plot, but with the first and last 100 y values trimmed.', fig.pos='H', message=FALSE>>=
...
p1 <- qplot(y, value, data = br111_bullet) + theme_bw()
p2 <- qplot(y, value, data = tail(head(br111_bullet, n = -100), n = -100)) + theme_bw()

grid.arrange(p1, p2, nrow = 2)
@

<<loess_step34, echo=FALSE, fig.width=10, fig.height=7, out.width='\\linewidth', warning=FALSE, fig.cap='Steps 3 and 4 of the Loess fitting algorithm. The top panel is a plot of the smoothed height values, and the bottom panel is a plot of the detected minimums.', fig.pos='H', message=FALSE>>=
...
p3 <- qplot(y, smoothed, data = tail(head(subdata, n = -10), n = -10)) + theme_bw()
...
p4 <- qplot(data=curdata, y, value) +
        theme_bw() +
        geom_vline(xintercept = curdata$y[min.left], colour = "red") +
        geom_vline(xintercept = curdata$y[min.right + (length(curdata$y) %/% 2)], colour = "blue")

grid.arrange(p3, p4, nrow = 2)
@

<<loess_step5, echo=FALSE, fig.width=10, fig.height=7, out.width='\\linewidth', warning=FALSE, fig.cap='Step 5 of the Loess fitting algorithm. The plot shows the result after trimming values below the first minimum, and above the second minimum.', fig.pos='H', message=FALSE>>=
...
qplot(y, resid, data=melt.df, #colour=factor(x), 
  ...
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_line(data = actual.df, aes(y, resid), colour="black", size=1, alpha=0.2) + ggtitle("Part 1 of Bullet 1 in black")
@    

\section{Application}
We've implemented a web interface to automate many of the steps outlined in the previous section. The web interface, developed using Shiny \cite{shiny}, allows a user to upload two x3p files representing scans of a bullet between two grooves. A 3D viewer, using Plotly \cite{plotly}, allows the user to rotate and zoom to examine surface features of the images. A screenshot of the application is available in Figure \ref{fig:app}. The application is available at the url \url{http://erichare.shinyapps.io/x3prproto}.
...
\begin{figure}
<<crosscuts-vary, echo=FALSE, fig.width = 14, fig.height = 21, out.width = '\\linewidth', cache=TRUE, warning = FALSE>>=
paths <- file.path(datadir, dir(datadir))
...
\hh{XXX Does this give us a way to automatically check for the bottom of the bullet? -- We could try to take two cross sections - maybe 50 or 100 $\mu m$ apart. If they don't match, we need to go a bit higher with the lower cross cut. Attempt at this is shown in Figure~\ref{fig:crosscuts2}.}
...
grid.arrange(lop[[1]], lop[[2]], lop[[3]], 
             ...
             ncol=1)
@
\caption{\label{fig:crosscuts2}Varying crosscuts for barrel 3, bullet 1. Initially, the match between crosscuts 25$\mu m$ apart is affected strongly by break off at the bottom of the bullet. At a crosscut level of $175\mu m$ the bullet pattern stabilizes. For this land, matches should not be attempted to lower crosscuts.}
\end{figure}
