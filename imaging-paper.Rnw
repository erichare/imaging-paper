...
\end{subfigure}
\caption{\label{fig:fourflags}}
\end{figure}
%
Using the James Hamby study, we run all unknown bullets against all known bullets for matches. 
The algorithm is using land-to-land matches, i.e. we are running $15 \times 6 = 90$ lands from unkown bullets against $2 \times 10 \times 6 = 120$ lands from known bullets, yielding a total of $10,800$ comparisons with 180 known land-to-land matches. 
