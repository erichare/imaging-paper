...
smooths <- rbind(peaks1$plot$data, peaks2$plot$data, 
  ...
  ylab(expression(paste("Crosscut values (in ",mu,"m)", sep=""))) 
@
\caption{\label{fig:varysmooth} Peak/valley detection at smoothing factors of 5, 25, and 35, respectively. Note that a smoothing factor of 5 yields enough noise that many very minimal overlapping peaks and valleys are detected, while a smoothing factor of 35 might over-smooth and cause the peaks/valleys to either disappear or shift  horizontally  from their position in the original loess residuals.}
\end{figure}
