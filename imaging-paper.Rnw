...
% Methodology
Throughout this section, we will work with images from the James Hamby Consecutively Rifled Ruger Barrel Study \cite{hamby:2009}. Ten Ruger P-85 pistol barrels were obtained from the manufacturer and fired to produce known test bullets and unknown bullets for comparison. 
3D topographical images are provided on the NIST Ballistics Database Project\footnote{\url{http://www.nist.gov/forensics/ballisticsdb/hamby-consecutively-rifled-barrels.cfm}} in a format called x3p (XML 3-D Surface Profile). 
...
x3p was adopted by the OpenFMC (Open Forensic Metrology Consortium), a group of academic, industry, and government Firearm Forensics researchers whose aim is to establish best practices for researchers using metrology in the forensic sciences.

In the Hamby Study, x3p files were created using a NanoFocus lens at 20x magnification. \hh{not sure where this sentences best goes.}

Each fired bullet is provided as a set of six x3p files, where each file is a surface scan between two bullet grooves, called a ``land". An example of plotting one of these lands is given in Figures~\ref{fig:sidex3p} and~\ref{fig:topx3p}, which show side and top profiles of the land respectively.
...
<<fixedX, dependson='data', echo=FALSE, warning=FALSE, message=FALSE, fig.width=6, out.width='\\textwidth', fig.pos='t!', fig.cap='Side profile of the height values of a bullet land across fixed x values. Note that the global features dominate any deviations, which would correspond to markings.'>>=
...
It's clear that our next approach must be to model the signal in order to focus on the noise, given that the noise should correspond to bullet markings.
\hh{XXX let's not call the markings noise - that has the conotation of them being random. The markings are also a signal, it is just dominated by the stronger signal of the land structure. We often go through a process of multiple steps to get close to the more interesting, smaller signal. }
\hh{XXX on a side note - I've got a different relative path to the x3p bullet files ... should we assume a data directory inside the folder? Then we don't have to push the files on a github repo again, but are using the same file structure?}
