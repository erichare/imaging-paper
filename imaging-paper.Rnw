...
rocs$variable <- factor(rocs$variable, levels=features)
qplot(`FALSE`, `TRUE`, data=rocs,  geom="line", group=type) + theme_bw() + geom_abline(linetype=2) + xlab("False negative rate") + 
  ylab("False positive rate") + facet_wrap(facets=~variable, ncol=4)
@
\caption{\label{fig:rocs}ROC curves for all of the features described in section~\ref{sec:algorithm}. \hh{XXX compare ROC curve of CMS against the other individual features -- ccf should be a lot more predictive than CMS. XXX}
\hh{XXX next steps: get rid of lower curve. find area under the curve, put value on bottom right of the chart, sort by ROC.}}
\end{figure}
