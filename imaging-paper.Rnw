...
imp$sdMatch <- sprintf("(%.2f)", imp$sdMatch)
imp$meanNonMatch <- round(imp$meanNonMatch,1)
imp$sdNonMatch <- sprintf("(%.2f)", imp$sdNonMatch)

print(xtable(imp, align="clrrrrr"), include.rownames=FALSE, floating = FALSE)
@
