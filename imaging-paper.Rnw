...
As mentioned, the algorithm for peak/valley detection depends on the selection of a smoothing window, called the smoothing factor. In particular, for detecting and removing the grooves prior to fitting a Loess regression, and for detecting the peaks/valleys of the loess residuals, we selected a smoothing factor of 35. We can investigate the performance of peak/valley detection across different smoothing factors. Figure \ref{fig:varysmooth} displays the resulting peaks and valleys across smoothing factors of 35, 5, and 65 respectively. It is immediately clear that a small smoothing factor like 5 is a poor choice. This results in a significant amount of noise in the data such that even just a point or two can skew the rolling average enough for a peak or valley to be detected. Given that the striation patterns are typically much larger, we're in effect muddying the waters by performing such minimal smoothing.

A larger smoothing factor on the other hand, like 65, seems to be a more plausible option. Most of the peaks/valleys present which are detected by a smoothing factor of 35 are also detected at 65. However, some notable issues arise. Notice that the peak on the left hand side of the image is smoothed out, and thus not detected. On the right hand side, the peak is just barely still present and hence detected. But in many cases, the large windows are smoothing out some of the structure that we wish to see. Furthermore, it can be seen that the peaks/valleys are often shifted relative to their position in the original loess residuals, or in the smoothed data with smaller smoothing factors.

\begin{figure}[hbtp]
%\centering
%\begin{subfigure}[t]{\linewidth}
%\caption{Peak and valley detection for bullet 1-5 at a smoothing factor of 35.\label{fig:smoothfac1}}{%
<<smoothfac1, echo=FALSE, fig.width=7, fig.height=6, out.width='\\linewidth', warning=FALSE>>=
br111.groove <- get_grooves(br111_bullet)
...
Figure~\ref{fig:cms} shows the strong connection between the maximal number of consecutive striae and matches in the Hamby study. All 29 pairs of lands with at least sixteen CMS in common are matches. There are two things that should be noted at this point: the automated algorithm finds a relatively high number of CMS even for non-matches. On average, there are 2.2 maximal CMS between known non-matches (with a standard deviation of 1.4). Known matches share on average 8.2 maximal CMS, with a standard deviation of 5.8. Secondly, a large number of maximal CMS by itself is not indicative of a match, as was previously pointed out by Miller~\cite{miller:1998}. Figure~\ref{fig:mismatch} shows a known mismatch, between two lands that share fifteen consecutively matched striae. The number of peaks and valleys, their heights and depths as well as the overall pattern are in relative good agreement. \hh{XXX something re-assuring that visually we can tell them apart? I don't know - this could just as well be a case where the crosscuts are very different. }
%
\begin{figure}[hbtp]
\begin{subfigure}[t]{\linewidth}
...
CCFs <- read.csv("data-15-25/bullet-stats.csv")
...
#  scale_shape_manual(values=c(1,2)) +
  ...
  theme(legend.position = "bottom")
@
\caption{\label{fig:treeresult}Overall predictions for each bullet based on the decision tree. Both results for the tree as well as the random forest are shown. For each bullet at least four lands are correctly matched. Unknown bullets B and M are the hardest for the algorithm to match.}
\end{figure}
%
...
imp$meanMatch <- sapply(imp$Variable, function(var) {
  mean(subset(CCFs, match)[,var])
})
imp$sdMatch <- sapply(imp$Variable, function(var) {
  sd(subset(CCFs, match)[,var])
})
...
The rows in the table are ordered according to descending importance. The variable with the most predictive power by far is cross-correlation (ccf), followed by the number of matches (num.matches) and the Euclidean distance between the crosscuts (distr.dist). The maximal consecutively matching striae (cms) is found only in seventh position.


\section{Conclusion}

We have presented an algorithm which detects the most prominent but least relevant structure of a bullet from a ballistics identification perspective, removes these features, and produces residuals which allows for the easy identification of markings. We've generalized this algorithm to align the residuals from two bullets to automatically determine whether they are matches. We created a random forest model using many of the features to provide a probabilistic assessment of the quality of a match, along with the most relevant features. Finally, we've provided an easy-to-use open-source backend, and web frontend to many of these features.

The matching algorithm is sensitive to the parameter choices made. The distance between crosscuts (currently 25$\mu m$) to evaluate stability, as well as the cross-correlation factor (currently 0.9) we set as a minimum are affecting the final outcome. Another parameter is the amount of smoothing in identifying peaks and grooves (currently a window of 57.8375$\mu m$ is used, corresponding to a window of 17 values to the left and the right of an observation). While we tried to lay out in the paper the impact each of the parameter choices has on the matching, we are still far from an optimized scenario. 
