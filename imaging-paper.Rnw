...
ratios <- dens.stats %>% group_by(var, x) %>% summarise(
  upper = sum(up[variable=="KM"])/sum(up[variable=="KNM"]),
  lower = sum(down[variable=="KM"])/sum(down[variable=="KNM"])
)
rm <- melt(ratios, measure.var=c("upper", "lower"))
levels(rm$variable) <- c("P(X < c | KM) / P(X < c | KNM)", "P(X > c | KM) / P(X > c | KNM)")
qplot(x, value, colour=variable,  data=rm) + facet_wrap(~var, scales="free") + scale_y_log10() + theme_bw() + geom_hline(yintercept=1) + scale_colour_brewer(palette="Set2") + ylab("Probability Ratios")
@
