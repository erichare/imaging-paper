...
For this, assume that $n$ data points $(x_1, y_1), (x_2, y_2), ..., (x_n, y_n)$ are given that are (approximately) located on a circle. We want to estimate the location of the center and radius of the best fitting circle using a least squares approach.
...
@
\end{subfigure}
\caption{\label{fig:residual} Residual structure of  circular fits at two different cross-section. The residual structure in (b) shows systematic structure, indicating that the fit is not entirely appropriate. }
\end{figure}
