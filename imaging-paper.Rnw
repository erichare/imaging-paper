...
If we perform a similar routine for part 2 of bullet 1, we obtain the figure give in Figure \ref{fig:circlefits2}. 
...
qplot(y, resid, data=subset(db2, x <= 80 & x >=75), colour=factor(x)) + 
  ...
  theme_bw() + theme(legend.position="bottom") + 
  geom_point(aes(y, resid), colour="black", size=1, alpha=0.2, 
             data = filter(db1, part==2, x <= 80, x >=75)[,c("y", "resid")]) + ggtitle("Part 2 of Bullet 1 in black")
@
...
To improve upon the previous algorithm, the first step must be to eliminate the grooves from the images under the assumption that they do not contain relevant information for determining a match. Fortunately, the location and appearance of the grooves in these 2d plots is quite consistent. Because a bullet is scanned groove to groove, the groove should appear within approximately scan points from each end. That is, on y coordinate system ranging from 0 to 1000 as these images, are, the grooves will appear between $y \in \{0, 100\}$ and $y \in \{900, 1000\}$. Hence, the algorith for removing the grooves can be described as follows:

1. Fix an x coordinate value.
2. Remove values from $y \in \{0, 100\}$ and $y \in \{900, 1000\}$.
3. For each y value from 100 to 900, compute a rolling average to smooth out any deviations occurring near the minimums.
4. Determine the minimum in the first half (corresponding to the left groove) and the minimum in the right half (corresponding to the right groove)
5. Remove all y values below the minimum and above the maximum
