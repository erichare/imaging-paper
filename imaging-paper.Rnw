...
\begin{figure}[hbtp]
  \centering
<<circlefits, echo=FALSE, fig.width=7, fig.height=10.5, out.width='\\linewidth', warning=FALSE>>=
...
\begin{enumerate}
    ...
    \item Remove values from $y \in \{0, 100\}$ and $y \in \{900, 1000\}$ (Figure~\ref{fig:loess_step12}).
    \item For each y value from 100 to 900, compute a rolling average to smooth out any deviations occurring near the minima. (Figure~\ref{fig:loess_step34}).
    \item Determine the minimum in the first half (corresponding to the left groove) and the minimum in the right half (corresponding to the right groove) (Figure~\ref{fig:loess_step34}).
...
#p2 <- qplot(y, value, data = tail(head(br111_bullet, n = -100), n = -100)) + theme_bw()

#grid.arrange(p1, p2, nrow = 3)
@
\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step34}Steps 3 - 5 of the Loess fitting algorithm. After smoothing the $y$ values left and right minima are detected (marked by vertical lines). Values outside these boundaries are removed (shown in grey)}
<<loess_step34, echo=FALSE, fig.width=10, fig.height=3.5, out.width='\\linewidth', warning=FALSE,  message=FALSE>>=
subdata <- tail(head(br111_bullet, n = -100), n = -100)
subdata$smoothed <- c(rep(NA, 10), rollapply(subdata$value, 21, function(x) mean(x, na.rm = TRUE)), rep(NA, 10))
p3 <- qplot(y, smoothed, data = tail(head(subdata, n = -10), n = -10)) + theme_bw() + xlim(c(0,2500))

curdata <- tail(head(subdata, n = -10), n = -10)
min.left <- which.min(curdata$value[1:(length(curdata$value) %/% 2)])
min.right <- which.min(curdata$value[(length(curdata$value) %/% 2):length(curdata$value)])

p4 <- qplot(data=curdata, y, value, size=I(1), colour=I("grey60"), alpha=I(.25)) +
        theme_bw() +
        geom_vline(xintercept = curdata$y[min.left], colour = "red") +
        geom_vline(xintercept = curdata$y[min.right + (length(curdata$y) %/% 2)], colour = "blue") + xlim(c(0,2500))


#grid.arrange(p3, p4, nrow = 2)
final.dat <- subset(curdata, y > curdata$y[min.left + 5] & y < curdata$y[min.right + (length(curdata$y) %/% 2) - 5])

p4 + geom_point(data=final.dat, aes(y, value))
@
\end{subfigure}
%' \begin{subfigure}[t]{\linewidth}
...
<<loess_fit, echo=FALSE, fig.width=10, fig.height=7, out.width='\\linewidth', warning=FALSE, fig.cap='Loess fit for part 1 of bullet 1', fig.pos='hbtp', message=FALSE>>=
...
<<loess_resid, echo=FALSE, fig.width=10, fig.height=7, out.width='\\linewidth', warning=FALSE, fig.cap='Loess fit residuals for part 1 of bullet 1', fig.pos='hbtp'>>=
my.loess <- fit_loess(br111_bullet, br111.groove)

my.loess$resid
@

We can reproduce a similar figure to the one from the circular fits by overlaying the residuals from each loess fit for bullet 2 onto an image of the loess residuals from part 1 of bullet 1. This is available in Figure \ref{fig:loessfits}. 

<<loessfits, echo=FALSE, fig.width=10, fig.height=7, out.width='\\linewidth', warning=FALSE>>=
...
qplot(y, resid, data=melt.df, #colour=factor(x), 
  ...
  theme_bw() + theme(legend.position="bottom") + 
  geom_line(data = actual.df, aes(y, resid), colour="black", size=1) + ggtitle("Part 1 of Bullet 1 in black")
@    
