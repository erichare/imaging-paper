...
\begin{figure}
<<crosscuts-vary, echo=FALSE, fig.width = 14, fig.height = 21, out.width = '\\linewidth', cache=TRUE, warnings = FALSE>>=
paths <- file.path(datadir, dir(datadir))
crosscuts <- seq(100, 450, by = 25)
lof <- processBullets(paths[5], x = crosscuts)
lof$bullet <- paste(lof$bullet, lof$x)
qplot(data=lof, x=y, y=resid, colour=factor(x), facets=~x)
...
grid.arrange(lop[[1]], lop[[2]], lop[[3]], lop[[4]], lop[[5]], lop[[6]], 
             lop[[7]], lop[[8]], lop[[9]], lop[[10]], lop[[11]], lop[[12]], 
             lop[[13]], lop[[14]],
             ncol = 2)
@
\caption{\label{fig:crosscuts}Overview of the variations in the surface along different crosscuts. Crosscut at $x = 100$ is compared to crosscuts every 25$\mu m$. \hh{XXX result?}}
\end{figure}
