...
qplot(y, resid, data=subset(db2, x == 100), #colour=factor(x), 
      geom="line", size=I(1), colour=I("grey70")) + 
  facet_wrap(~part, ncol=2) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_line(aes(y, resid, group = x), colour="black", size=1, 
             data = filter(db1, part==1, x == 100)[,c("y", "resid", "x")]) +
  ylab("Residuals from circular fit") +
  ggtitle("Part 1 of Bullet 1 in black")
@
\caption{Circle fit for each cross section of bullet 2, with part 1 of bullet 1 overlaid.\label{fig:circlefits}}%
\end{figure}
...
\begin{enumerate}
    \item Fix an $x$ coordinate value (Figure~\ref{fig:loess_step12}).
    \item For each y value, compute a rolling average to smooth out any deviations occurring near the minima. (Figure~\ref{fig:loess_step12}).
    \item Determine the peak of the left groove by finding the first value with a negative discretized derivative. Determine the valley of the left groove by finding the first value with a positive discretized derivative after the value of the peak. (Figure~\ref{fig:loess_step34}).
    \item Determine the peak of the right groove by finding the last value with a positive discretized derivative. Determine the valley of the right groove by finding the last value with a negative discretized derivative before the value of the peak. (Figure~\ref{fig:loess_step34}).
    \item Remove all $y$ values left and right of the identified minima in vertical direction (Figure \ref{fig:loess_step34}).
\end{enumerate}

Next, we fit a Loess regression to the data. Figure \ref{fig:loess_fit} provides a look at the Loess fit, in blue, overlaid against the processed image of part 1 of bullet 1. The fit seems to do a reasonable job of capturing the structure of the image. Figure \ref{fig:loess_resid} shows the residuals from this fit. It is clear that there is a rather significant amount of autocorrelation, which make sense given the presence of striations and markings. Certainly, tuning the parameters for this regression, such as the Loess span, could prove to do an even better job at making the markings more clear. \hh{XXX did you follow up on that? What is the effect of using a different span?}

\begin{figure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step12}Steps 1 and 2 of the Loess fit algorithm. For a fixed crosscut (x held constant at 243.75) height values for part 1 of bullet 1 are plotted across different values of y. The height values are smoothed to remove noise in the deviations.}
<<loess_step12, echo=FALSE, fig.width=10, fig.height=3.5, out.width='\\linewidth', warning=FALSE,  message=FALSE>>=
br111_bullet <- get_bullet(paste(datadir,"Br1 Bullet 1-1.x3p", sep = "/"))
...
subdata$smoothed <- c(rep(NA, 10), rollapply(subdata$value, 21, function(x) mean(x, na.rm = TRUE)), rep(NA, 10))
subdata$smoothed2 <- c(rep(NA, 10), rollapply(subdata$smoothed, 21, function(x) mean(x, na.rm = TRUE)), rep(NA, 10))
p2 <- qplot(y, smoothed2, data = subdata) + theme_bw() + xlim(c(0,2500))

p2
@
\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step34}Steps 3 - 5 of the Loess fitting algorithm. After smoothing the $y$ values left and right minima are detected (marked by vertical lines, red indicating peaks and blue indicating valleys). Values outside these boundaries are removed (shown in grey)}
<<loess_step34, echo=FALSE, fig.width=10, fig.height=3.5, out.width='\\linewidth', warning=FALSE,  message=FALSE>>=
peak_ind <- head(which(diff(subdata$smoothed2) < 0), n = 1)
    groove_ind <- head(which(diff(tail(subdata$smoothed2, n = -(peak_ind + 10))) > 0), n = 1) + peak_ind + 10
    
    peak_ind2 <- tail(which(diff(subdata$smoothed2) > 0), n = 1)
    groove_ind2 <- tail(which(diff(head(subdata$smoothed2, n = -(length(subdata$smoothed2) - peak_ind2 + 10))) < 0), n = 1)
    
    p4 <- qplot(subdata$y[subdata$y < subdata$y[groove_ind]], subdata$smoothed2[subdata$y < subdata$y[groove_ind]], colour=I("grey60"), alpha=I(.25)) +
        ...
        geom_point(aes(x, y), data = data.frame(x = subdata$y[subdata$y > subdata$y[groove_ind2]], y = subdata$smoothed2[subdata$y > subdata$y[groove_ind2]]), colour=I("grey60"), alpha=I(.25)) +
        geom_point(inherit.aes = FALSE, aes(x, y), data = data.frame(x = subdata$y[subdata$y < subdata$y[groove_ind2] & subdata$y > subdata$y[groove_ind]], y = subdata$smoothed2[subdata$y < subdata$y[groove_ind2] & subdata$y > subdata$y[groove_ind]])) +
        theme_bw() +
...
\end{subfigure}
%' \begin{subfigure}[t]{\linewidth}
%' \caption{\label{fig:loess_step5}Step 5 of the Loess fitting algorithm. The plot shows the result after trimming values below the first minimum, and above the second minimum.}
%' <<loess_step5, echo=FALSE, fig.width=10, fig.height=3.5, out.width='\\linewidth', warning=FALSE,  message=FALSE>>=
%' final.dat <- subset(curdata, y > curdata$y[min.left + 5] & y < curdata$y[min.right + (length(curdata$y) %/% 2) - 5])
%' 
%' qplot(data=final.dat, y, value) +
%'     theme_bw() + xlim(c(0,2500))
%' @
%' \end{subfigure}
\caption{Overview of all five steps of the loess algorithm to identify and remove big bullet strucure.}
\end{figure}

\begin{figure}[hbtp]
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_fit} Loess fit for land 1 of bullet 1.}
...
\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_resid} Residuals of Loess fit for land 1 of bullet 1.}
<<loess_resid, echo=FALSE, fig.width=10, fig.height=3.5, out.width='\\linewidth', warning=FALSE>>=
my.loess$resid
@
\end{subfigure}
\caption{Fit and residuals of a loess fit to land 1 of bullet 1 (Barrel 1).}
\end{figure}

We can reproduce a similar figure to the one from the circular fits by overlaying the residuals from each loess fit for bullet 2 onto an image of the loess residuals from part 1 of bullet 1. This is shown in Figure~\ref{fig:loessfits}. 
...
qplot(y, resid, data=melt.df, #colour=factor(x), 
  ...
  facet_wrap(~part, ncol=2) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_line(data = actual.df, aes(y, resid), colour="black", size=1) + ggtitle("Land 1 of Bullet 1 in black")
@    
\caption{Residuals of a loess fit to bullets 1 and 2 from barrel 1. Land 1 of bullet 1 is shown in black. }
\end{figure}
...
%'   ylim(c(0,15))
%' @
%' \caption{\label{fig:thresholds}Varying the threshold levels results in different number of consecutive matching striae between the two bullets. By default, the threshold with the highest CMS is chosen automatically.}
%' \end{figure}

\subsection{Varying crosscuts}
The crosscut at which a comparison is done matters -- as crosscuts are further apart, more pronounced differences between the crosscuts show up. For bullet land 1-5 from barrel 1 crosscuts at intervals of 50$\mu m$ between 150$\mu m$  and 450$\mu m$  are compared to a crosscut at 100$\mu m$. The initially almost perfect match (consolidated by a CMS of nineteen) between the  crosscuts deteriorates with increasing distance to a very questionable match at $x = 350\mu m$ and non-matches further apart. Only if the crosscut is within 200$\mu m$ do we get a good visual match. Given that alignment in the microscope is not perfect, we should take this into account in the automatic matching routine by evaluating matches at several cross sections. 
\begin{figure}
...
\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{All bullet pairs with a maximum of at least nine CMS are matches.}
...
Using recursive partitioning, we find a decision tree predicting matches based on features derived from the image files. The resulting tree is shown in Figure~\ref{fig:tree}. A total of 93 lands are being matched correctly. Interestingly, the number of consecutive matching striae is not featured in this evaluation. Instead, cross-correlation (ccf) between the crosscuts is featured heavily. Aside from cross correlation, the number of matches between crosscuts is involved in the decision. 
This is not in contradiction to earlier papers emphasizing the value of CMS on visual assessments of bullet matches. 
...
library(xtable)
xtable(xtabs(~I(pred35>0.5)+match, data=CCFs))
@
