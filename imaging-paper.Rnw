...
\subsection{Identifying grooves}
...
% We have implemented a web interface to automate many of the steps outlined in the previous section. The web interface, developed using Shiny~\citep{shiny}, allows a user to upload two x3p files representing scans of a bullet between two grooves. A 3D viewer, using Plotly~\citep{plotly}, allows the user to rotate and zoom to examine surface features of the images. A screenshot of the application is available in Figure \ref{fig:app}. The application is available at the url \url{http://erichare.shinyapps.io/x3prproto}.
% 
% \begin{figure*}
...
% \end{enumerate}
% 
% The application includes a button which allows the automatic creation of a residual plot from the loess fits of two bullets. An x coordinate value can be selected as a cross section, and cross sections of the two bullets at the x value are taken. The previously described loess regression procedure is applied to the two cross sections. A residual plot is produced beneath the surface plots allowing for an assessment of the similarity of the markings. Users need to perform this procedure at different x values in order to see whether the markings indicate a match at different points along the bullet.
% 
...
% %'   br111 <- get_crosscut(paths[5], x = x)
...
% %' @
% 
% \includegraphics[width=.7\columnwidth]{images/side-by-side-manual-match.png}
% \caption{\label{fig:manualmatch}Manually adjusted side-by-side comparison of bullet 1-5 and bullet 2-1.}
% \end{figure}

\begin{figure}[hbtp]
\centering
\includegraphics[width=.7\columnwidth]{images/matchup-rgl.png}
...
\end{subfigure}    
\begin{subfigure}[b]{.49\textwidth}\centering
    \caption{Aligned (by horizontal shift) signatures.\label{fig:crosscutX}}{%
...
\begin{figure}[hbtp]
  \centering
<<ccf, echo=FALSE, fig.width = 7, fig.height = 3, out.width = '.5\\textwidth'>>=
lag <- ccf$lag[which.max(ccf$acf)]
...
@
\caption{\label{fig:ccf}Cross-correlation function between the two signatures shown in Figure~\ref{fig:crosscut} at lags between -100 and 100. At a lag of $\Sexpr{lag}$ the correlation peaks, indicating the largest amount of agreement between the signatures. Figure~\ref{fig:crosscutX} shows the lag-shifted signatures.}
\end{figure}
...
However, we can extract signatures from multiple cross sections of the same bullet land for an initial assessment of its quality. \hh{XXX shift cross section discussion into the supplement XXX}
...
by comparing signatures from cross sections that are not too far apart --  25 $\mu m$ to 50 $\mu m$ -- we get an indication whether the signatures come from a rapidly changing section of the surface, indicative of a break-off \hh{or some other damage}, or from a stable section, where we can have a reasonable expectation of finding matches to other signatures. In the approach here, we keep increasing the cross section until we find a section with a stable pattern. This process is shown in Figure~\ref{fig:crosscuts2} at the example of bullet 1-1 from barrel 3, where
...
<<smooth, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3.5, out.width='.5\\textwidth', warning=FALSE>>=
...
<<smoothcut, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3.5, out.width='.5\\textwidth', warning=FALSE>>=
#images <- file.path(datadir, dir(datadir))
  lof <- processBullets(paths = images[c(5,7)], x = 100)
...
<<smoothmatch, echo=FALSE, dependson='twolands100match', fig.width=7, fig.height=2.75, out.width='.5\\textwidth', warning=FALSE>>=
peaks1$lines$bullet <- b12[1]
peaks2$lines$bullet <- b12[2]
...
\begin{enumerate}
\item {\bf Smooth the two cross sections} by a loess with a very small span (see Figure~\ref{fig:smooth}). 
\item Use cross-correlation to {\bf find the best alignment} along the cross sections: Shift one of the cross sections by the lag indicated by the cross-correlation function (see Figure~\ref{fig:ccf} for the cross-correlation function and Figure~\ref{fig:crosscutX} for the resulting shift).
\item Using a rolling average, {\bf identify peaks and grooves} for each of the cross sections. We define a region around the extrema on each side as one third of the distance to the next extrema (see Figure~\ref{fig:smoothcutb}). 
\item {\bf Match grooves and peaks between cross sections:} based on the regions around the extrema as defined above, we identify joint regions between two cross sections as those areas, in which at least two of the individual regions touch. A joint region is defined as the smallest interval that covers all of the touching regions. A joint region is called a match between the cross sections, if all of the cross section-specific regions are of the same type of extrema, i.e.\ either all peaks or all valleys. In Figure~\ref{fig:match} all matches are colored corresponding to their type of extrema. Non-matching regions are left grey.
\item {\bf Identify the maximal number of CMS} (consecutive matching striae) between the two cross sections by counting consecutive matching lines. 
In the example of Figure~\ref{fig:match}, the number of consecutive matching striations (CMS) is fifteen, a  high number indicative of a match between the bullets. For lead bullets, such as this, Biasotti~\citep{biasotti:1959} considered four or more consecutive lines to be sufficient evidence of a match, and which are part of current practice in assessing bullet matches~\citep{nichols:1997, nichols:2003, nichols:2003b}.
\end{enumerate}
...
\begin{figure}[hbtp]
  \centering
\begin{subfigure}[t]{\textwidth}\centering
\caption{\label{fig:distr.dist}Relationship between maximal CMS and distances between cross sections.}
<<distr.dist, echo=FALSE, fig.width=8, fig.height=4, out.width='.5\\textwidth'>>=
ggplot(data=bstats) + 
...
ggplot(data=bstats) + 
    geom_density(aes(x=distr.dist, fill=match), alpha=0.5) +
...
x <- seq(0,20, by=0.02)
km <- sm.density(bstats$distr.dist[bstats$match], display="none", eval.points=x)
knm <- sm.density(bstats$distr.dist[!bstats$match], display="none", eval.points=x)
dframe <- data.frame(x = x, km=km$estimate, knm=knm$estimate)
dframe$kmtail <- rev(cumsum(rev(dframe$km)))/sum(dframe$km)
dframe$knmtail <- rev(cumsum(rev(dframe$knm)))/sum(dframe$knm)
...
includesVar <- setdiff(names(bstats), c("b1", "b2", "data", "resID", "id.x", "id.y", "pred", "forest", "bullet", "span", "crosscutdist"))
