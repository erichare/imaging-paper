...
\begin{figure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_step12}Steps 1 and 2 of the Loess fit algorithm. For a fixed crosscut (x held constant at 243.75) height values for part 1 of bullet 1 are plotted across different values of y. The first and last 100 values of y are trimmed (shown in grey).}
...
p1 <- qplot(y, value, data = br111_bullet, size=I(1), colour=I("grey60"), alpha=I(.25)) + theme_bw()
p1 + geom_point(aes(y, value), data = tail(head(br111_bullet, n = -100), n = -100))
...
\end{subfigure}
\begin{subfigure}[t]{\linewidth}
\caption{\label{fig:loess_resid} Loess fit residuals for land 1 of bullet 1.}
...
@
\end{subfigure}
\caption{\label{fig:hamby-perfect}Showcase scenario of when matching with CMS works perfectly. Unfortunately the matches are not always that convincing.}
\end{figure}
