...
Instead of CMS, cross-correlation (ccf) between the crosscuts is very important in the matching process  by the decision tree. Aside from  cross correlation, the number of matches (num.matches) is involved in the decision. 
...
#  scale_shape_manual(values=c(1,2)) +
  ...
  theme(legend.position = "bottom")
@
\caption{\label{fig:treeresult}Overall predictions for each bullet based on the decision tree (maximum possible number is 12). Both results for the tree and the random forest are shown. For each bullet at least three lands are correctly matched. Unknown bullet B is the hardest for the algorithm to match.}
\end{figure}
%
