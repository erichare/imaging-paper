...
\hh{In an approach to analyze the striation pattern we extract a {\emph bullet profile} or {\emph signature} \cite{ma:2004} by taking a cross section across the surface measurements for a fixed value of $x$.}
...
Figure \ref{fig:fixedX} shows a plot of a bullet signature. It can be seen that the global structure of the land dominates the appearance of the plot. The grooves can be clearly identified on the left and right side, and the curvature of the surface is the most visible feature in the middle.
...
The smooth curve on the plot represents a segment of a perfect circle with the same radius as the bullet. \hh{While the circle is an obvious first choice for fitting the structure, it does not completely capture the bullet surface after it was fired. A discussion of the fit and the remaining residual structure can be found in the supplement.} \hh{XXX Reference to the supplement? XXX}
...
\hh{Instead, we use multiple loess fits to model the overall structure in order to identify the bullet markings. }

\subsection{Identify Grooves}
...
\begin{enumerate}
    \item Fix an $x$ value \hh{to extract a bullet signature} (Figure~\ref{fig:loess_step1}, with $x$ set to 243.75$\mu m$).
...
Applying the loess fit to a range of different surface cross cuts (see Figure~\ref{fig:manualmatch-rgl}, with cross cuts from 50 to 150) shows the striation marks from two bullets. Cross cuts of bullet 1 are shown on the left (cross cuts with values below 100) and cross cuts of bullets 2 are shown on the right (cross cuts with values above 100). Many of the striation marks align, allowing for an easy visual assessment of the shared similarities between the two bullets. 
...
\begin{figure}[hbtp]
  \centering
<<tree:forest, echo=FALSE, fig.width=6, fig.height=6.25, out.width='.5\\textwidth', warning = FALSE>>=
