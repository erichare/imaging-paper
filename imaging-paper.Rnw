...
\item \hh{XXX Alternative approaches: IBIS database and the NRC report against it\footnote{Fox report on the NRC report \url{http://www.foxnews.com/story/2008/03/05/nationwide-firearms-database-shot-down-by-government-sponsored-study.html}.
...
\begin{figure}[hbtp]
  \centering
    \subfloat[View from the side.\label{fig:sidex3p}]{%
      \includegraphics[width=\linewidth]{images/sidex3p.png}
    }
    \hfill
    \subfloat[View from the top.\label{fig:topx3p}]{%
    \includegraphics[width=\linewidth]{images/topx3p.png}
    }
\caption{An example of a scan of a bullet land.  \hh{XXX is the code for these pictures somewhere in the code folder? Let's put a link/note to it here. The aspect ratio of these 3D files should be adjusted such that it faithfully shows the actual geography. That should happen in the function plot3D.x3p.file }}
\end{figure}
...
\hh{The tilt of the lines to the left in Figure~\ref{fig:topx3p} is not an artefact, but a direct and expected consequence of the spin induced by the rifling during the firing process. Depending on whether a barrel is rifled clock-wise or anti-clockwise, the striations have a left or right tilt. The direction of the rifling is a class characteristic. XXX rotation in Ruger? twist rate?}
...
<<fixedX, dependson='data', echo=FALSE, warning=FALSE, message=FALSE, fig.height=2.5, fig.width=6, out.width='\\linewidth', fig.pos='t!', fig.cap='Side profile of the height values of a bullet land across fixed x values. Note that the global features dominate any deviations, which would correspond to markings.'>>=
...
qplot(data=subset(dbr111, x <= 100*1.5625^2 & x >= 95*1.5625^2), y, value, colour=factor(x), geom="line", size=I(1)) +
...
qplot(data=subset(dbr111, x <= 80*1.5625^2 & x >=75*1.5625^2), y, value-ypred, colour=factor(x), geom="line", size=I(1)) +
...
qplot(data=subset(dbr111, x <= 5*1.5625^2), y, value-ypred, colour=factor(x),
      geom="line", size=I(1)) +
  scale_colour_brewer("x", palette="Paired") + 
  theme_bw() + 
  theme(legend.position="bottom")
...
qplot(y, resid, data=subset(db2, x <= 80*1.5625^2 & x >=75*1.5625^2), colour=factor(x), geom="line", size=I(1)) + 
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_line(aes(y, resid, group = x), colour="black", size=1, alpha=0.2, 
             data = filter(db1, part==1, x <= 80*1.5625^2, x >=75*1.5625^2)[,c("y", "resid", "x")]) + ggtitle("Part 1 of Bullet 1 in black")
@

If we perform a similar routine for part 2 of bullet 1, we obtain the figure given in Figure \ref{fig:circlefits2}. 

<<circlefits2, echo=FALSE, fig.width=10, fig.height=7, out.width='\\linewidth', warning=FALSE, fig.cap='Circle fit for each cross section of bullet 2, with part 2 of bullet 1 overlayed.', fig.pos='H', cache=TRUE>>=
qplot(y, resid, data=subset(db2, x <= 80*1.5625^2 & x >=75*1.5625^2), colour=factor(x), geom="line", size=I(1)) + 
  facet_wrap(~part) + scale_colour_brewer(palette="Paired") +
  theme_bw() + theme(legend.position="bottom") + 
  geom_line(aes(y, resid, group =x), colour="black", size=1, alpha=0.2, 
             data = filter(db1, part==2, x <= 80*1.5625^2, x >=75*1.5625^2)[,c("y", "resid", "x")]) + ggtitle("Part 2 of Bullet 1 in black")
@
...
\section{Application}
We've implemented a web interface to automate many of the steps outlined in the previous section. The web interface, developed using Shiny[ref], allows a user to upload two x3p files representing scans of a bullet between two grooves. A 3D viewer, using Plotly[ref], allows the user to rotate and zoom to examine surface features of the images. A screenshot of the application is available in Figure \ref{fig:app}.
...
\begin{figure}
\centering
    \subfloat[Crosscuts at $x = 100$.\label{fig:crosscut}]{%
<<crosscuts, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3, out.width='.49\\textwidth'>>=
...
@
    ...
    \hfill
    \subfloat[Adjusted (Bullet 1-5 horizontally shifted) crosscuts at $x = 100$.\label{fig:crosscutX}]{%
<<crosscutsX, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3, out.width='.49\\textwidth'>>=
...
@
    ...
    \hfill
    \subfloat[Values above/below a threshold level are defined as peaks and grooves. \label{fig:smoothcutb}]{%
<<smoothcut, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=4, out.width='.49\\textwidth', warning=FALSE>>=
...
#  geom_point(aes(y=r05)) +
  ...
  scale_colour_brewer(palette="Set2", na.value = "grey90") +
  theme_bw() +
  theme(legend.position="bottom")
@
    }
\hfill
    \subfloat[Overlaps of the same color indicate matching striation marks.  \label{fig:smoothcutc}]{%
<<smoothtile, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=2.5, out.width='.49\\textwidth'>>=
qplot(data=lofX, x=y, y=bullet, fill=type, geom="tile") +
...
\hh{Figure~\ref{fig:match} gives an overview of the automated matching routine: after smoothing the scan with a loess of a very small span, shown in (a), we define a striation mark as a groove or a peak if it exceeds a pre-set threshold (of 0.75$\mu m$; XXX need to vary over this threshold level next). Two lines are defined as a match, if their peak/groove base at the threshold overlaps. If there is no overlap, a peak/groove leads to a mismatch. We then count the number of maximum consecutive matches. }
