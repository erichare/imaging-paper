...
<<smooth, echo=FALSE, dependson='twolands100', fig.width=7, fig.height=3, out.width='.49\\textwidth', warning=FALSE>>=
...
qplot(data=lofX, y, resid, colour=bullet, size=I(1)) +
  geom_line(aes(y=l30), colour="grey30") +
  facet_grid(bullet~.) +
...
qplot(data=lofX, x=y, y=l30, colour=type, geom="line", group=bullet) + 
  facet_grid(bullet~.) +
...
\caption{\label{fig:smoothcut} Processing surface crosscuts.}
\end{figure}
