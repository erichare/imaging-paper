...
In an approach to analyze the striation pattern we extract a \emph{bullet profile} \citep{ma:2004} by taking a cross section across the surface measurements for a fixed value of $x$.
...
Instead of a circular fit, we use multiple loess fits to model the overall structure in order to identify the bullet markings. 
...
\begin{enumerate}
    \item Fix an $x$ value to extract a bullet profile (Figure~\ref{fig:loess_step1}, with $x$ set to 243.75$\mu m$).
...
Figure~\ref{fig:loess_fit} provides a look at the loess fit, in blue, overlaid against the processed image of bullet~1-5. The fit seems to do a reasonable job of capturing the structure of the image. The blue area around the signature are  95\% bootstrapped confidence intervals based on 1,000 bootstrap samples. 
...
In this section, we are going to describe the algorithm for matching signatures first, and discuss the impact of paramater choices in the subsections thereafter.
...
We first identify a stable region for each bullet and pick the signature at the lowest cross section level in this region, because typically, individual characteristics are best expressed at the lower end of the bullet (see supplement~\ref{supp:bulletbottom} for a more detailed discussion). 
...
\hh{XXX the smoothing factor needs a few more details: a smoothing factor of 5 means that we consider the two measurements on the left and the right of the observations under consideration XXX}
Figure~\ref{fig:varysmooth} displays the  peaks and valleys detected in the same signature at smoothing factors of 5, 25, and 45, respectively. The dark line corresponds to the smoothed values, while the grey line in the back shows the raw signature. The choice of smoothing factor is a classical decision of a bias/variance trade-off. It is immediately clear that a small smoothing factor like 5 is a poor choice. It results in a significant amount of noise in the data such that even just a point or two can skew the rolling average enough for a peak or valley to be detected. Given that the striation patterns are typically much larger, we are in effect muddying the waters by performing such minimal smoothing. Another consideration is that the smoothing should not fall below the  resolution of the equipment at which the surface measurements are taken -- in order to not introduce artefacts in the analysis. 
...
\end{subfigure}
\caption{\label{fig:fourflags}Images of the four lands that got flagged during the quality assessment. All of them show scratch marks across the striation marks from the barrel. They will be excluded from the remainder of the analysis.}
\end{figure}
%
We exclude these four lands from the further matching considerations and 
...
%' \end{figure}
%' %
This makes the errors in the automated approach smaller than the human error in the Hamby study. Out of the 507 participants, who returned results, eight (out of $15 \times 507 = 7,605$) bullets were not matched conclusively, resulting in a false negative rate of 0.0011.
