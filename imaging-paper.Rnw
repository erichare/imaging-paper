...
\thankstext{t1}{Thanks to somebody \hh{XXX acknowledge the grant here? XXX}} 
...
It might not be reasonable to assume that we can match all lands, but the idea is to try to maximize the number of matches to get an overview of what we can reasonably expect from an automated match. 
...
\end{subfigure}
\begin{subfigure}[t]{.49\textwidth}\centering
\caption{Overlaid signatures of C-3 and the land with the second top matching CMS.}
...
\end{subfigure}
\begin{subfigure}[b]{\textwidth}\centering
\caption{Conditional barchart \citep{hummel}: heights show probability of match/non-match given a specific CMS. All land-to-land comparisons with at least 13 CMS are matches.}
<<cms-spines, echo=FALSE,  fig.width=7, fig.height=4, out.width='.5\\textwidth'>>=
ggplot(data=subset(bstats, !flagged)) + 
...
\end{subfigure}
\begin{subfigure}[t]{\textwidth}\centering
\caption{Densities of maximum CMS for Known Matches (KM) and Known Non Matches (KNM).}
<<cms-densities, echo=FALSE,  fig.width=7, fig.height=3.5, out.width='.5\\textwidth'>>=
x <- seq(0,29, by=0.03)
km <- sm.density(bstats$CMS[bstats$match], display="none", eval.points=x)
knm <- sm.density(bstats$CMS[!bstats$match], display="none", eval.points=x)
...
\begin{figure}[hbtp]
  \centering
<<strange-res, echo=FALSE, fig.width=7, fig.height=3.25, out.width='.5\\textwidth', warning=FALSE>>=
...
includesVar <- setdiff(names(bstats), c("b1", "b2", "data", "resID", "id.x", "id.y", "pred", "forest", "bullet", "span", "crosscutdist", "flagged"))
...
\end{figure}
%
After about 25 trees, the error rate of land-to-land comparisons stabilizes at 0.0040. This is a weighted average between a false positive error rate of $9.8 \times 10^{-5}$ and an error rate of false negatives of 0.22. \hh{This out-of-bag error rate is over-estimating the actual error in the Hamby study: here, the final random forest based on 300 trees is able to correctly predict all known matches and non-matches (see Figures~\ref{fig:treeresult} and ~\ref{fig:tree-forest}).}
Note that this error rate is based on land-to-land comparisons and is much lower for bullet-to-bullet matches. In the case of the Hamby study, the overall error rate is zero, if we require for a bullet match that at least two of a bullet's lands are matched (see Figure~\ref{fig:treeresult}). 
\begin{figure}[hbtp]
  \centering
...
Resources for this manuscript, including R code to reproduce any of the models, tables and figures, can be found at  \url{https://github.com/erichare/imaging-paper}.
