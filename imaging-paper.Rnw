...
\hfill
    \subfloat[Grey backgrounds identify a line on one of the bullets. Overlaps of the same color indicate matching striation marks.
    Matching striations are marked by an `o', mismatches are marked by an `x'.   \label{fig:smoothcutd}]{%
<<smoothmatch, echo=FALSE, dependson='twolands100match', fig.width=7, fig.height=2.75, out.width='.49\\textwidth'>>=
ggplot() +
