...
<<smoothfac1, echo=FALSE, fig.width=7, fig.height=3, out.width='\\textwidth', warning=FALSE>>=
br111.groove <- get_grooves(br111_bullet)
br111.loess <- fit_loess(br111_bullet, br111.groove)
get_peaks(br111.loess$data, smoothfactor = 35)$plot
@
    }
\end{subfigure}    
\begin{subfigure}[t]{\linewidth}
...
We have presented ...
\hh{XXX Quick summary of the main points of the paper XXX }
...
For a fair assessment of the adequacy of an algorithm we need transparency. Our matching  algorithm is open: the code is readily available in form of the R package x3rplus~\cite{x3prplus}, and the code to produce this paper is available at \hh{XXX, Eric ... we need a website} \eh{XXX can we just put the GitHub link here?}. Obviously, this is only a first step, but in times of the Netflix challenge and the many Data Science challenges hosted by Kaggle, we should not let the quality of the matching algorithm be the problem that stands in the way.
