...
bstats <- read.csv("data/bullet-stats.csv")
ggplot(data=bstats) + 
...
\begin{figure}
<<cms-outlier, echo=FALSE, fig.width=7, fig.height=4, out.width='\\linewidth'>>=
idx <- which(bstats$cms==11 & !bstats$match)[1]
load(as.character(bstats$data[idx]))
res <- reslist[[bstats$resID[idx]]]
print(ggplot() +
  theme_bw() + 
  geom_rect(aes(xmin=xmin, xmax=xmax), ymin=-6, ymax=5, fill="grey90", data=res$lines) +
  geom_line(aes(x = y, y = l30, colour = bullet),  data = res$bullets) +
#  geom_hline(yintercept = res$threshold) +
#  geom_hline(yintercept = - res$threshold) +
  scale_colour_brewer(palette="Set1") +
  ...
  geom_text(aes(x = meany), y= -5.5, label= "o", data = subset(res$lines, match)))
@
\caption{Wrongfully identified match between two bullet lands with a maximum CMS of 11.}
\end{figure}
