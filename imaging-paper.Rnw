...
Figure~\ref{fig:cms} shows the strong connection between the maximal number of consecutive striae and matches in the Hamby study. All 23 pairs of lands with at least twelve CMS in common are matches. For smaller number of CMS, the percentage of false positives quickly increases. Taking cross-correlation into account (see Figure~\ref{ccfs-match}) increases the number of matches. A cross-correlation of above 0.75 in the Hamby study is indicative of a match. There are 67 land-land comparisons with a cross-correlation of above 0.75.   
\begin{figure}
\begin{subfigure}[t]{\linewidth}
