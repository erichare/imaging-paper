...
We first identify a stable region for each bullet and pick the signature at the lowest cross section level in this region, because typically, individual characteristics are best expressed at the lower end of the bullet (see supplement section~\ref{supp:bulletbottom} for a more detailed discussion). 
...
\begin{figure}[hbtp]
  \centering
<<tree, echo=FALSE, fig.width=7, fig.height=4, out.width='.65\\textwidth'>>=
library(rpart)
library(rpart.plot)
library(RColorBrewer)
...
\begin{figure}[hbtp]
  \centering
<<oob, echo=FALSE, fig.width=6, fig.height=3, out.width='.6\\textwidth'>>=
