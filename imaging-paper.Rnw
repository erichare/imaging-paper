...
\item The analysis by Miller \cite{miller:1998} involved comparisons of 50 pairs of bullets against IBIS results (at the time consisting of 1,600 bullets of the same characteristics).  

The feasibility of a database of ballistic images was evaluated in a 2008 report \cite{nap:2008} by the National Research Council. Part of the evaluation was based  on Integrated Ballistics Identification System (IBIS) -- to be precise, on the IBIS Heritage Line, which is based on 2d microscopic images. The findings of the report are 

While microscopy allows some 3d effects (using different lighting and the innate ability of our eyes to move focus), in images this is reduced to a color shade of a pixel at best. This means that out of the top six features in Table~\ref{tab:importance}, only the number of matches and mismatches are available for a match, which vastly reduces the quality of automated matches. 
