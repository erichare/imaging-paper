...
\initial{F}\textbf{orensic examiners try to determine whether two bullets were fired from the same barrel by analyzing them under a microscope in controlled conditions. Can we make the task simpler?}
...
Figure~\ref{fig:crosscuts} shows a sequence of crosscuts for bullet 1-5 (barrel 1) that are taken 50$\mu m$ apart, between 150$\mu m$  and 450$\mu m$. These are compared to a crosscut at 100$\mu m$. Initially this comparison constitutes an almost perfect match between the two crosscuts. However, the match quickly deteriorates with increasing distance to non-matches at a crosscut of $x = 300\mu m$ or higher.  Only if the crosscut is within 150$\mu m$ do we get a good visual match even when we know that the same bullet surface is being used. 
...
We first identify a stable region for each bullet and pick the lowest crosscut level in this region, because typically, individual characteristics are best expressed at the lower end of the bullet. 
