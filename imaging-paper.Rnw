...
\hh{As a first step, we automatically assess the quality of each of the lands by  checking that we can identify a stable region on each land. For this, we use cross sections 25$\mu m$ apart and require a minimum of 0.9 for the cross correlation. Four lands from different bullets are flagged as problematic in this respect. A visual inspection (see Figure~\ref{fig:fourflags}) shows that each one of these lands shows scratch marks across the surface.}
%
\begin{figure}
  \centering
\begin{subfigure}[t]{.49\textwidth}\centering
...
<<cms-spines, echo=FALSE,  fig.width=7, fig.height=3.5, out.width='.5\\textwidth'>>=
ggplot(data=subset(bstats, !flagged)) + 
