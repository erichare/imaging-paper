...
\caption{Maximal number of CMS between unknown Bullet D land 5 and all of the 120 other (known) lands. For two lands the number of maximum CMS is high. }
<<cms, echo=FALSE, fig.width=7, fig.height=3.5, out.width='\\textwidth'>>=
load("data/unkn17.RData")
...
\end{subfigure}
\begin{subfigure}[t]{.49\textwidth}
\caption{Overlaid lines of the land of the unknown bullet D-5 and the land with the top matching CMS.}
...
\caption{Overlaid lines of the land of the unknown bullet D-5 and the land with the second top matching CMS.}
...
Figure~\ref{fig:cms} shows the strong connection between the maximal number of consecutive striae and matches in the Hamby study. All 28 pairs of lands with at least nine CMS in common are matches. For smaller number of CMS, the percentage of false positives quickly increases. 
\begin{figure}
\begin{subfigure}[t]{\linewidth}
...
\begin{figure}
<<distr.dist, echo=FALSE, fig.width=7, fig.height=4, out.width='\\linewidth'>>=
ggplot(data=bstats) + 
    geom_jitter(aes(x=factor(cms), y=distr.dist, colour=match)) +
    theme(legend.position="bottom") + 
    xlab("maximum CMS")
@
