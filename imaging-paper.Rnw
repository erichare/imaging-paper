...
This defines the {\it signature} of bullet~1-5. Blue areas show  95\% bootstrap confidence intervals (based on $B=500$ bootstrap samples).
}
...
grid.arrange(lop[[1]], lop[[2]], lop[[3]], 
             ...
             ncol=2)
@
\caption{\label{fig:crosscuts2}Varying cross sections for barrel 3, bullet~1-1. Initially, the match between cross sections 25$\mu m$ apart is affected strongly by some break off at the bottom of the bullet. At a cross section level of $175\mu m$ the bullet pattern stabilizes. For this land, matches should not be attempted at lower cross sections. }
\end{figure}
...
\begin{figure}[hbtp]
  \centering
<<density-overview,echo=FALSE, warning=FALSE, fig.width=12, fig.height=7, out.width='\\textwidth'>>=
...
@
\caption{\label{fig:densities}Overview of all the marginal densities for features extracted in section~\ref{sec:algorithm}.}
\end{figure}
...
Instead of CMS, cross-correlation (ccf) between the cross sections is very important in the matching process  by the decision tree. Aside from  cross correlation, the number of matches is involved in the decision. 
