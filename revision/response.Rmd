...
1. Read the paper start to finish
2. Look at boxplots vs densities (maybe use jittered boxplots?)
...
***We selected random forests because the method has desirable attributes and can be easily ported to other forensic applications.  The focus of the paper was not to find the best algorithm to develop a score but rather show the potential of combining as large a number of attributes as are available to discriminate among bullets.  ***
...
*** AC:  Actually I think that the meaning is different.  I wonder whether ``closed system'' not in the sense of secret but rather in the sense of building a big database of signatures that can be searched (like AFIS, but open).  This is what I interpret given the question about how forensic scientists might use this method. *** 

***TODO: Focus on open algorithms (opening up the closed system) / merge responses***
...
***TODO: Cite riva paper, explain work has been done on cartridges (not bullets)***
...
*** AC:  The reviewer is correct.  Here, we talk about ``match'' only as a means to test whether our algorithm is actually pairing the samples that are true pairs.  This is a term used only in this development stage and is not intended for the court. ****

***TODO: Clarify that we are discussing "match" only in terms of assessing our algorithm. Merge AC's response***
