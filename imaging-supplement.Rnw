...
qplot(x=y, y=l30, data=aligned, geom="line", group=bullet, colour=bullet) + 
  ylim(c(-7,7)) + xlab("") + ylab("Signature") + 
  facet_grid(facets=barrel~land, labeller="label_both") + theme_bw() +
  theme(plot.margin=unit(c(0,0,0,0), unit="cm"), legend.position="none") + 
  scale_colour_manual(values=cols) +
  geom_label(aes(x=x, y=y, label=label), data=labels, 
             inherit.aes = FALSE)
@
...
imp <- data.frame(importance(rtrees2))
