...
\hh{
add: we could extend the results of the James Hamby study by trying to match every single land against all other lands. This would almost double the number of comparisons ($21,115 = (118+88)\cdot 205/2$).  
}
\hh{Show aligned signatures for all ten barrels. }
<<aligned, echo=FALSE, warning=FALSE, fig.width=8, fig.height=10, out.width='\\textwidth', cache=TRUE>>=
...
qplot(x=y, y=resid, data=crosscuts, geom="line", group=path) + ylim(c(-7,7)) + facet_wrap(facets=~id, ncol=6) + theme_bw() 
@
