...
\section{Signature intensities}

\hh{XXX Overview of the signatures across one bullet land. Maybe with color?}



Standard deviation of signatures is one measure to quantify the extent to which they are expressed. Figure~\ref{fig:crosscuts} shows the amount of the standard deviation across crosscuts for all bullet lands shown as lines. Standard deviation decreases as the crosscut increases, i.e.\ bullets' signatures are most expressed close to the bottom of the bullet. \hh{XXX can we find an AFTE citation to back this up?}

\begin{figure}[hbtp]
\centering
<<crosscuts-sd, echo=FALSE, fig.height=5, fig.width=8, out.width='.7\\textwidth'>>=
...
@
\caption{\label{fig:crosscuts}Standard deviation reduces as crosscut levels increase.}
\end{figure}
