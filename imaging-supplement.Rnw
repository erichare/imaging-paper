...
Figure~\ref{fig:fixedX} shows the side profile of a crosscut through the surface measurements of bullet 1-5 at a fixed level. The smooth line on top is a circle, with estimated radius and center. The details of this fit are given below. We then remove the fit to investigate residuals.
%
\begin{figure}[hbtp]
  \centering
...
The residuals are dominated, as expected, by the grooves, which show up as large positive residuals. For a surface cross cut at $x = 100$ there is a residual circular structure that does not show up for all signatures. 
...
A single cylinder as a fit is unlikely to be a particularly good fit, because there seem to be quite massive deformations in the vertical direction. Even when we fit a circle at each cross section of the bullet, as in Figure~\ref{fig:circlefits}, this does not address all of these issues. While the wider circumference at the base of the bullet can be resolved by individual circular fits, the systematic residual structure in Figure~\ref{fig:residualb} stays the same.
...
\newpage
\section{Assessing cross-correlation between signatures at multiple cross sections}\label{supp:ccf}

Figure~\ref{fig:crosscuts} shows a sequence of signatures for bullet 1-5 (barrel 1) that are taken at cross sections 50$\mu m$ apart, between 150$\mu m$  and 400$\mu m$. These are compared to the signature at cross section 100$\mu m$. Initially, this comparison constitutes an almost perfect match between the two signatures. However, the match quickly deteriorates with increasing distance between the cross sections.  Only if signatures are from cross sections  within 150$\mu m$ do we get a good visual match even when we know that the same bullet surface is being used. 
...
grid.arrange(lop[[1]], lop[[2]], lop[[3]], lop[[4]], lop[[5]], lop[[6]],
             ncol = 2)
@
\caption{\label{fig:crosscuts}Overview of the variations in the surface along different cross sections. The signature extracted at $x = 100$ is compared to signatures at every 50$\mu m$. With every step away from the original cross section site, the number of differences between the signatures increases; the number of maximum CMS decreases from initially 22 to  four or fewer at $x = 300\mu m$ and higher. }
\end{figure}

\newpage
\section{Signature intensities}\label{supp:bulletbottom}
<<setup-signatures, echo=FALSE>>=
library(x3pr)
library(x3prplus)
library(dplyr)
library(ggplot2)
...
@
%
Figure~\ref{fig:overview} shows an overview of the signatures across different crossucts on a single bullet. 
\begin{figure}[hbtp]
...
@
\caption{\label{fig:overview}Signatures of the same bullet at different crosscuts. With increasing crosscut level, peaks and valleys are less pronounced, resulting in a smaller standard deviation.}
\end{figure}
As crosscut levels increase individual characteristics become less distinctive, making true matches to other bullets harder. The pattern of decreasing peaks and valleys is generally true for  bullet lands, as can be seen in Figure~\ref{fig:sds}. 
%
\begin{figure}[hbtp]
\centering
...
qplot(x, sd, data=stats, geom="line", group=bullet, alpha=I(0.5)) + theme_bw() +
  geom_smooth(group=1) + 
  xlab("Crosscut location") +
  ylab("Standard deviation")
@
\caption{\label{fig:sds}Standard deviation reduces as crosscut levels increase.}
\end{figure}
Figure~\ref{fig:sds} shows that the amount of standard deviation of a signature decreases on average for all bullet lands as crosscut levels increase.
This makes standard deviation of a signature one measure to quantify the extent to which a signature is expressed. This implies that for identifying matches we should use the lowest crosscut level to extract a bullet's signature once a stable surface region is detected. This is in accordance with current standard practice \citep{afte:1992}.

\newpage
\section{Complete evaluation of the Hamby study}\label{supp:extended}
\hh{One way to expand the use of the James Hamby study is to not only match all of the unknown bullet lands against the known bullet lands, but to compare every land against every other land. This effectively doubles the number of comparisons from 10,384 pairwise comparisons of useable bullet lands to 21,115 ($= (118+88)\cdot 205/2$) comparisons by adding another 10,731 bullet land comparisons made up of known-to-known and unknown-to-unkown comparisons. }

\hh{When we predict the new 10,731 comparisons using the random forest based on the previously considered 10,384 known-unknown comparisons, we encounter 18 false negatives and 9 false positives, corresponding to an actual false error rate of 0.19 and a false positive rate of 0.00085, which is close to the estimated error rates of 0.2267 and 0.0002. }

\hh{However, if we use all of the available comparisons to fit another random forest of 300 trees, the de-facto error rates for false positives and false negatives are again at 0. The estimated OOB error rates are 0.00024 for the false positive rate and 0.22180 for the false negative rate.}
...
qplot(x=y, y=l30, data=aligned, geom="line", group=bullet) + ylim(c(-7,7)) + facet_grid(facets=barrel~land, labeller="label_both") + theme_bw() +
  xlab("") + ylab("Signature") + 
  theme(plot.margin=unit(c(0,0,0,0), unit="cm"))
@
\caption{\label{fig:aligned}Overview of aligned signatures for all bullet lands for the ten barrels of the Hamby study. }
\end{figure}
