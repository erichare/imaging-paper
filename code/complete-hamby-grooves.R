...
bullets_processed <- lapply(all_bullets, function(bul) {
    ...
    my.groove <- grooves[grooves$bullet == bul$path,]
    
    processBullets(bullet = bul, name = bul$path, x = ccs$cc[which(ccs$path == bul$path)], grooves = c(as.numeric(my.groove[2]), as.numeric(my.groove[3])))
})
